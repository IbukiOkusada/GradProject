...
HRESULT CSpeedMeter::Init(void)
{
	...
	if (m_pMeter == nullptr)
	{
		...
		m_pMeter->SetPosition(METER::POS);
		m_pMeter->SetSize(METER::WIDTH, METER::HEIGHT);

		m_pMeter->BindTexture(CManager::GetInstance()->GetTexture()->Regist(METER::TEX_PATH));
		m_pMeter->SetDraw();
	}
...
void CSpeedMeter::Needle(void)
{
	...
	CDebugProc::GetInstance()->Print("[針の向き:%f]", fRot);
}