...
namespace
{
	...
	const float MOUSE_ROTATESPEED_X = (0.005f);		// マウス回転速度x軸
...
void CCamera::Pursue(const D3DXVECTOR3 pos, const D3DXVECTOR3 rot, const float fLength)
{
	...
	//目的の注視点の座標を取得
	posRDest = D3DXVECTOR3(pos.x - sinf(rottemp.y) * 200.0f, pos.y + 20.0f + (40.0f * (1.0f - m_fZoom)), pos.z - cosf(rottemp.y) * 200.0f);
